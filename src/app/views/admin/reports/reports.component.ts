...
export class ReportsComponent implements OnInit {
  ...
  renderFastMovingProductsChart(): void {
    ...
    const fastMovingProducts = this._transactionCalculator.getFastMovingProducts();
  
    const productNames = fastMovingProducts.map(product => product.productName);
    ...
    const option = {
      ...
      tooltip: {
        trigger: 'item',
        formatter: '<b>{a}:</b> {c} <br/> <b>Product Name:</b> {b} <br/> <b>Quantity Sold:</b> {d} ',
      },
      legend: {
        orient: 'horizontal',
        bottom: 'bottom',
